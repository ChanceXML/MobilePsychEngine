    ...
    private function keysCheck():Void
{
		...
		public function initLuaShader(name:String, ?glslVersion:Int = 120)
	{
		...
		{
			// prepend absolute external storage path if it's missing
			var pathPrefix:String = "";
			#if android
			if (folder.indexOf("/storage/") == -1) 
			{
				pathPrefix = "/storage/emulated/0/Android/data/com.shadowmario.psychengine/files/"; // android path
			}
			#end

			var frag:String = pathPrefix + folder + name + '.frag';
			var vert:String = pathPrefix + folder + name + '.vert';
